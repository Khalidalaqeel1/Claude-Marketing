# Browser-agent prompt: build the lead ad sets + 10 ads in the Ads Manager UI (all OFF)

Why: the Meta Ads connector can't create lead ad sets for the Suitiee Page (subcode 1815089), so this is built by hand in Ads Manager.
Run it in Claude in Chrome, logged in as Khalid. Say "انشر النموذج" in that chat only if the form should be published.
Image hashes for the 10 creatives are listed in `live-setup.md`.
